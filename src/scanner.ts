...
export async function scanPackages(names: string[]): Promise<Record<string, string>> {
  const result: Record<string, string> = {};

  for (const name of names) {
    const msg = packages.get(name);
    if (msg) {
      result[name] = msg;
    }
  }
...
export async function scan(json?: PackageJson): Promise<Record<string, string>> {
  if (!json) {
    try {
  ...
  return await scanPackages([...deps]);
}
